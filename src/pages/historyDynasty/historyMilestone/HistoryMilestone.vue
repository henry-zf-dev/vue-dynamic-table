<template>
  <div class="history-milestone-page" style="padding-top: 60px">
  ...
  export default {
    ...
    data() {
      return {
        pageTitle: '重大事件',
        tableLoading: true,
        tableId: tableIdConfig.historyMilestone,
        columns: [],
        filters: [],
        tableData: []
      };
    ...
    methods: {
      ...
      getTableData() {
        ...
        API.historyMilestone.historyMilestoneList(params).then(resp => {
      ...
      deleteTableItem(data = {}) {
        this.$confirm(`确认要删除${this.pageTitle} ${data.name} 吗？`, '提示', {
          ...
          API.historyMilestone.historyMilestoneDelete({id: data.id}).then(resp => {
            if (checkRespCorrect(resp)) {
