<template>
  <div class="dynasty-progress-page" style="padding-top: 60px">
  ...
  export default {
    ...
    data() {
      return {
        pageTitle: '朝代',
        tableLoading: true,
        tableId: tableIdConfig.dynastyProgress,
        columns: [],
        filters: [],
        tableData: []
      };
    ...
    methods: {
      ...
      getTableData() {
        ...
        API.dynastyProgress.dynastyProgressList(params).then(resp => {
      ...
      deleteTableItem(data = {}) {
        this.$confirm(`确认要删除${this.pageTitle} ${data.name} 吗？`, '提示', {
          ...
          API.dynastyProgress.dynastyProgressDelete({id: data.id}).then(resp => {
            if (checkRespCorrect(resp)) {
