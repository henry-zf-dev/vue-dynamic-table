    ...
    <div :class="{'page-container': !showTableOnly}">
      <div :class="{'table-container': !showTableOnly}">
        <EZTable
          ...
          <div slot="empty">
            <EmptyContent :opt-data="selfCheckEmptyBtn"></EmptyContent>
          </div>
          <div v-if="showTableOnly" slot="customOpts">
            <el-tooltip :open-delay="300" content="新增房间自检规则" placement="top">
              <el-button
                class="iconfont icon-tianjia table-filter-opt-btn table-filter-opt-text-btn"
                @click="selfCheckAdd">&nbsp;添加规则</el-button>
            </el-tooltip>
          </div>
        </EZTable>
      </div>
...
export default {
      ...
      selfCheckEmptyBtn: [
        {
          ...
          label: '添加自检',
          icon: 'icon-tianjia',
          perm: permissionConfig.monitorRoomSelfCheckManage,
          callback: () => {
            this.selfCheckAdd();
          }
        }
  ...
  methods: {
    ...
    selfCheckAdd() { // 新增房间自检
      this.$router.push({
        name: routerMeta.monitorRoomSelfCheckAdd.name,
        params: {
          rid: this.roomId
        }
      });
    ...
    selfCheckDetail(row) { // 自检详情
      this.$router.push({
        name: routerMeta.monitorRoomSelfCheckDetail.name,
        params: {
          rid: this.roomId,
          sid: row.id
        }
    ...
    setAlarm(row) { // 设为告警
      this.$router.push({
        name: routerMeta.monitorRoomSelfCheckSetAlarm.name,
        params: {
          sid: row.id,
          rid: this.roomId
        }
