  ...
  export default {
    ...
    data() {
      return {
        pageTitle: '文字书法',
        tableLoading: true,
        tableId: tableIdConfig.characterCalligraphy,
        columns: [],
        filters: [],
        tableData: []
      };
    ...
    methods: {
      ...
      getTableData() {
        ...
        API.characterCalligraphy.characterCalligraphyList(params).then(resp => {
      ...
      deleteTableItem(data = {}) {
        this.$confirm(`确认要删除${this.pageTitle} ${data.name} 吗？`, '提示', {
          ...
          API.characterCalligraphy.characterCalligraphyDelete({id: data.id}).then(resp => {
            if (checkRespCorrect(resp)) {
