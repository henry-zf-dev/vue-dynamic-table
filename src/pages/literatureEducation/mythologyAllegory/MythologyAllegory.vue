  ...
  export default {
    ...
    data() {
      return {
        pageTitle: '神话寓言',
        tableLoading: true,
        tableId: tableIdConfig.mythologyAllegory,
        columns: [],
        filters: [],
        tableData: []
      };
    ...
    methods: {
      ...
      getTableData() {
        ...
        API.mythologyAllegory.mythologyAllegoryList(params).then(resp => {
      ...
      deleteTableItem(data = {}) {
        this.$confirm(`确认要删除${this.pageTitle} ${data.name} 吗？`, '提示', {
          ...
          API.mythologyAllegory.mythologyAllegoryDelete({id: data.id}).then(resp => {
            if (checkRespCorrect(resp)) {
