  ...
  export default {
    ...
    data() {
      return {
        pageTitle: '文学著作',
        tableLoading: true,
        tableId: tableIdConfig.famousLiterature,
        columns: [],
        filters: [],
        tableData: []
      };
    ...
    methods: {
      ...
      getTableData() {
        ...
        API.famousLiterature.famousLiteratureList(params).then(resp => {
      ...
      deleteTableItem(data = {}) {
        this.$confirm(`确认要删除${this.pageTitle} ${data.name} 吗？`, '提示', {
          ...
          API.famousLiterature.famousLiteratureDelete({id: data.id}).then(resp => {
            if (checkRespCorrect(resp)) {
