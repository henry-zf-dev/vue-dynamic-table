  ...
  export default {
    ...
    data() {
      return {
        pageTitle: '思想流派',
        tableLoading: true,
        tableId: tableIdConfig.thoughtFactions,
        columns: [],
        filters: [],
        tableData: []
      };
    ...
    methods: {
      ...
      getTableData() {
        ...
        API.thoughtFactions.thoughtFactionsList(params).then(resp => {
      ...
      deleteTableItem(data = {}) {
        this.$confirm(`确认要删除${this.pageTitle} ${data.name} 吗？`, '提示', {
          ...
          API.thoughtFactions.thoughtFactionsDelete({id: data.id}).then(resp => {
            if (checkRespCorrect(resp)) {
