<template>
  <div class="great-thinker-page" style="padding-top: 60px">
  ...
  export default {
    ...
    data() {
      return {
        pageTitle: '大思想家',
        tableLoading: true,
        tableId: tableIdConfig.greatThinker,
        columns: [],
        filters: [],
        tableData: []
      };
    ...
    methods: {
      ...
      getTableData() {
        ...
        API.greatThinker.greatThinkerList(params).then(resp => {
      ...
      deleteTableItem(data = {}) {
        this.$confirm(`确认要删除${this.pageTitle} ${data.name} 吗？`, '提示', {
          ...
          API.greatThinker.greatThinkerDelete({id: data.id}).then(resp => {
            if (checkRespCorrect(resp)) {
