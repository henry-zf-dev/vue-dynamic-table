    ...
    <div class="page-container">
      <div class="table-container">
        <EZTable
          ...
          <div slot="empty">
            <EmptyContent :opt-data="joinSelfEmptyBtn"></EmptyContent>
          </div>
        </EZTable>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    jointCheckRuleAdd() { // 房间联检新增
      this.$router.push({
        name: routerMeta.systemRoomJointCheckRuleListAdd.name,
        params: {
          rid: this.roomId,
          isCheck: 1
        }
    ...
    jointCheckRuleDetail(row) { // 房间联检查看
      this.$router.push({
        name: routerMeta.systemRoomJointCheckRuleListDetail.name,
        params: {
          rid: this.roomId,
          jid: row.id,
          isCheck: 1
