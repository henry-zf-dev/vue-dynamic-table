  ...
  export default {
    ...
    data() {
      return {
        pageTitle: '名山大川',
        tableLoading: true,
        tableId: tableIdConfig.mountainsRivers,
        columns: [],
        filters: [],
        tableData: []
      };
    ...
    methods: {
      ...
      getTableData() {
        ...
        API.mountainsRivers.mountainsRiversList(params).then(resp => {
      ...
      deleteTableItem(data = {}) {
        this.$confirm(`确认要删除${this.pageTitle} ${data.name} 吗？`, '提示', {
          ...
          API.mountainsRivers.mountainsRiversDelete({id: data.id}).then(resp => {
            if (checkRespCorrect(resp)) {
