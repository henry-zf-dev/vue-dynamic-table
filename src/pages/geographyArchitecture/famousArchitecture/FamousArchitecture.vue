<template>
  <div class="famous-architecture-page" style="padding-top: 60px">
  ...
  export default {
    ...
    data() {
      return {
        pageTitle: '著名建筑',
        tableLoading: true,
        tableId: tableIdConfig.famousArchitecture,
        columns: [],
        filters: [],
        tableData: []
      };
    ...
    methods: {
      ...
      getTableData() {
        ...
        API.famousArchitecture.famousArchitectureList(params).then(resp => {
      ...
      deleteTableItem(data = {}) {
        this.$confirm(`确认要删除${this.pageTitle} ${data.name} 吗？`, '提示', {
          ...
          API.famousArchitecture.famousArchitectureDelete({id: data.id}).then(resp => {
            if (checkRespCorrect(resp)) {
