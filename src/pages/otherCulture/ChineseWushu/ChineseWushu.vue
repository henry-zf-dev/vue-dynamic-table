<template>
  <div class="Chinese-Wushu-page" style="padding-top: 60px">
  ...
  export default {
    ...
    data() {
      return {
        pageTitle: '中华武术',
        tableLoading: true,
        tableId: tableIdConfig.ChineseWushu,
        columns: [],
        filters: [],
        tableData: []
      };
    ...
    methods: {
      ...
      getTableData() {
        ...
        API.ChineseWushu.ChineseWushuList(params).then(resp => {
      ...
      deleteTableItem(data = {}) {
        this.$confirm(`确认要删除${this.pageTitle} ${data.name} 吗？`, '提示', {
          ...
          API.ChineseWushu.ChineseWushuDelete({id: data.id}).then(resp => {
            if (checkRespCorrect(resp)) {
