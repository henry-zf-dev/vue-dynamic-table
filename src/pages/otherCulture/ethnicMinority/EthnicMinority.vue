<template>
  <div class="ethnic-minority-page" style="padding-top: 60px">
  ...
  export default {
    ...
    data() {
      return {
        pageTitle: '少数民族',
        tableLoading: true,
        tableId: tableIdConfig.ethnicMinority,
        columns: [],
        filters: [],
        tableData: []
      };
    ...
    methods: {
      ...
      getTableData() {
        ...
        API.ethnicMinority.ethnicMinorityList(params).then(resp => {
      ...
      deleteTableItem(data = {}) {
        this.$confirm(`确认要删除${this.pageTitle} ${data.name} 吗？`, '提示', {
          ...
          API.ethnicMinority.ethnicMinorityDelete({id: data.id}).then(resp => {
            if (checkRespCorrect(resp)) {
