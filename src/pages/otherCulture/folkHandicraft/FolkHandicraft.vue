  ...
  export default {
    ...
    data() {
      return {
        pageTitle: '民间工艺',
        tableLoading: true,
        tableId: tableIdConfig.folkHandicraft,
        columns: [],
        filters: [],
        tableData: []
      };
    ...
    methods: {
      ...
      getTableData() {
        ...
        API.folkHandicraft.folkHandicraftList(params).then(resp => {
      ...
      deleteTableItem(data = {}) {
        this.$confirm(`确认要删除${this.pageTitle} ${data.name} 吗？`, '提示', {
          ...
          API.folkHandicraft.folkHandicraftDelete({id: data.id}).then(resp => {
            if (checkRespCorrect(resp)) {
