<template>
  <div class="zodiac-culture-page" style="padding-top: 60px">
  ...
  export default {
    ...
    data() {
      return {
        pageTitle: '生肖文化',
        tableLoading: true,
        tableId: tableIdConfig.zodiacCulture,
        columns: [],
        filters: [],
        tableData: []
      };
    ...
    methods: {
      ...
      getTableData() {
        ...
        API.zodiacCulture.zodiacCultureList(params).then(resp => {
      ...
      deleteTableItem(data = {}) {
        this.$confirm(`确认要删除${this.pageTitle} ${data.name} 吗？`, '提示', {
          ...
          API.zodiacCulture.zodiacCultureDelete({id: data.id}).then(resp => {
            if (checkRespCorrect(resp)) {
