<template>
  <div class="tradition-festival-page" style="padding-top: 60px">
  ...
  export default {
    ...
    data() {
      return {
        pageTitle: '传统节日',
        tableLoading: true,
        tableId: tableIdConfig.traditionFestival,
        columns: [],
        filters: [],
        tableData: []
      };
    ...
    methods: {
      ...
      getTableData() {
        ...
        API.traditionFestival.traditionFestivalList(params).then(resp => {
      ...
      deleteTableItem(data = {}) {
        this.$confirm(`确认要删除${this.pageTitle} ${data.name} 吗？`, '提示', {
          ...
          API.traditionFestival.traditionFestivalDelete({id: data.id}).then(resp => {
            if (checkRespCorrect(resp)) {
