<template>
  <div
    :style="{minHeight: fitTable ? `${tableMinHeight}px` : 'auto'}"
    ...
    <div :style="{paddingBottom: rows.length > 0 ? '65px' : ''}" class="table-content">
      <div v-if="rows.length === 0" class="table-content-empty">
...
<script>
import { getTableMinHeight, getTableState, updateTableState } from '../../utils';
...
export default {
  ...
  computed: {
    ...mapState(['clientHeight', 'tableState', 'routeParamState'])
  },
  watch: {
    selectionData() {
    ...
    clientHeight() {
      this.tableMinHeight = getTableMinHeight(this.tableId, 40);
    },
    tableState() {
